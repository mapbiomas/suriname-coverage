...
//---------------------TEMPORAL FILTER: PRIORITY EXECUTION STRATEGY-----------------------
/**
 * This section defines the **priority of land cover classes** when applying
 * temporal consistency corrections over the classification time series.
 * 
 * → Class 24 = Urban areas (true urban presence, often small and fragmented)
 * → Class 27 = Non-urban / residual class (could be natural areas, noise, or confusion)
 *
 * The goal is to correct temporal inconsistencies, e.g. alternating patterns:
 *     24 27 24 27 24 → corrected to → 24 24 24 24 24 (urban confirmed)
 *
 * However, to avoid forcing urban presence where it’s uncertain, we apply different strategies:
 *
 * - `order_exec_first = [27]`: At the **first year (1985)**, we are conservative.
 *     We assume urban areas detected only in that year without support in adjacent years
 *     may be noise. So, we **prioritize class 27** (non-urban), avoiding overestimation.
 *
 * - `order_exec_last = [24]`: At the **last year (2024)**, we prioritize keeping urban class (24),
 *     assuming that recent detections of urban expansion are more trustworthy and should be preserved.
 *
 * - `order_exec_middle = [24, 27, 24, 27]`: In the **intermediate years**, we alternate:
 *     first reinforcing urban areas where consistent (24), then allowing class 27 corrections.
 *     The repeated passes aim to stabilize both over-detections and under-detections.
 */

var ordem_exec_first  = [27];              // Conservative correction: remove false early urban areas
var ordem_exec_last   = [24];              // Preserve valid urban areas detected in recent years
var ordem_exec_middle = [24, 27, 24, 27];  // Alternate corrections in the middle years to smooth transitions
...
var prefixo_out = param.country+ '-' + param.code_region + '-' 
////*************************************************************
...
var Clasificacion_TD = ee.ImageCollection(dirinput)
                      .filterMetadata('description', 'equals', 'gapfill')  //
...
function getAreas(image, region) {
  ...
  bandNames.evaluate( function(bands, error) {
    ...
    Export.table.toDrive({
      ...
      description: 'LANDCOVER-STATS-'+prefixo_out+param.version_output,
      fileFormat: 'CSV',
      folder: 'P02_2-TEMPORAL-FILTER-CLASSIFICATION'
    });
